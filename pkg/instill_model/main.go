...
func (con *Connection) getServerURL() string {
	return fmt.Sprintf("%s", con.config.GetFields()["server_url"].GetStringValue())
}

func (con *Connection) getModelID() string {
	return fmt.Sprintf("%s", con.config.GetFields()["model_id"].GetStringValue())
}
